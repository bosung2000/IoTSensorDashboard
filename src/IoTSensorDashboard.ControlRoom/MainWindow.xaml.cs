...
public partial class MainWindow : Window
{
    ...
    private void UpdateIntegrityChip(MetricsSnapshot metrics)
    {
        ...
        else
        {
            ...
            //
            // 🔴 중복(I1 이 접은 것)을 **항상 함께 보여준다** — 실측에서 필요했다.
            //    이 문구는 중복을 검사하지 않아서, 들어온 것이 **전부 중복으로 접혀
            //    저장이 0** 이 되어도 「정합 OK」라고 말했다.
            //    중복은 원래 있는 것(재전송)이라 0 이 정상이 아니지만,
            //    **수신량에 육박하면 그건 사고**다. 그걸 볼 수 있어야 한다.
            IntegrityDot.Fill = Ok;
            IntegrityText.Text = $"정합 OK · 중복 {metrics.Duplicate:N0}";
        }
    }
